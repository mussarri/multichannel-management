...
async function logout(token) {
  const response = await fetch(
    `${ENV[CURRENT_ENV].BASE_URL}/earsiv-services/assos-login`,
    {
      ...
      referrer: `${ENV[CURRENT_ENV].BASE_URL}/intragiris.html`,
      body: `assoscmd=${
        CURRENT_ENV === "PROD" ? "anologin" : "logout"
...
async function createDraftInvoice(token, invoiceDetails = {}) {
  const invoiceData = {
    ...
    gelirVergisiTevkifatiTutari:
      invoiceDetails.gelirVergisiTevkifatiTutari || "0",
    bagkurTevkifatiOrani: invoiceDetails.bagkurTevkifatiOrani || 0,
    gelirVergisiTevkifatiTutari:
      invoiceDetails.gelirVergisiTevkifatiTutari || "0",
    ...
    ozelMatrahVergiTutari: (invoiceDetails.specialTaxBaseTaxAmount || 0)
      .toFixed(2)
      .toString(),
    ...
    malHizmetTable: invoiceDetails.items.map((item) => ({
      ...
      malHizmetTutari: ((item.quantity || 0) * (item.unitPrice || 0))
        .toFixed(2)
        .toString(),
...
async function getAllInvoicesIssuedToMeByDateRange(
  token,
  { startDate, endDate }
) {
  const invoices = await runCommand(
