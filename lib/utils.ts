...
export const findCityId = (cityName: string) => {
  return (
    il().findIndex(
      (item) => item.toLocaleLowerCase() === cityName.toLocaleLowerCase()
    ) + 1
  );
};

export const ilce = (il_id: string) => {
  const ilceler = [
    ...
    .filter((item) => item.il_id.toString() === il_id.toString())
    .map((item) => item.name);
  return ilceler;
};

export const slugify = function string_to_slug(str) {
